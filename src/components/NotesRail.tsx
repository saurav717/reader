...
interface Props {
  ...
  inWindow?: boolean;
  /** Opens another paper — from the list of every paper's notes — with its notes. */
...
export default function NotesRail({ paperId, selectedId, orphanIds, onSelect, onClose, onPopOut, inWindow = false, onOpenPaper }: Props) {
  const { papers, highlights, updateHighlight, deleteHighlight } = useStore();
  const paper = papers.find((item) => item.id === paperId);
  // Every paper's notes, a card to each, in place of this one's: with no paper
  // open there is nothing else to show.
  const [everyPaper, setEveryPaper] = useState(false);
  const showingAll = everyPaper || !paper;
  const others = useAllNotes().filter((summary) => summary.paperId !== paperId && papers.some((item) => item.id === summary.paperId)).length;
  ...
  const exportMarkdown = () => {
    if (paper) saveMarkdown(paper.title, toMarkdown(paper.title, mine, notes));
  };

  // Whose notes these are, and the way to every other paper's.
  const whose = (
    <div className={`notes-paper${inWindow ? ' in-window' : ''}`}>
      {showingAll ? (
        <span className="notes-paper-name">
          <span className="notes-paper-eyebrow">Every paper</span>
          <span className="notes-paper-title">Your notes, a paper to each</span>
        </span>
      ) : (
        <span className="notes-paper-name" title={paper?.title}>
          <span className="notes-paper-eyebrow">Notes on this paper</span>
          <span className="notes-paper-title">{paper?.title}</span>
        </span>
      )}
      {paper ? (
        <button
          type="button"
          className="btn ghost sm notes-paper-switch"
          aria-pressed={everyPaper}
          onClick={() => setEveryPaper(!everyPaper)}
          title={everyPaper ? 'Back to the notes on the paper open now' : "See every paper's notes, and switch to another"}
        >
          {everyPaper ? '← This paper' : `All notes${others ? ` · ${others + (notes.length ? 1 : 0)} papers` : ''}`}
        </button>
      ) : null}
    </div>
  );
  ...
  return (
    <aside className="panel notes-rail" aria-label="Highlights and notes">
      ...
      {showingAll ? (
        <NotesIndex current={paper?.id} onOpenPaper={(id) => (id === paperId ? setEveryPaper(false) : onOpenPaper?.(id))} />
      ) : (
        <>
          <div className="notes-tabs" role="tablist" aria-label="Notes or highlights">
