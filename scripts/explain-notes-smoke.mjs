...
check('seven pieces, in the order they were kept', labels.length === 7 && /^Diagram/.test(labels[0]) && /^(Code|Output)/.test(labels[1]) && labels[2] === 'Table' && labels[3] === 'Equation' && /^Caveat/.test(labels[4]) && labels[5] === 'Passage' && labels[6] === 'Section', labels.join(' | '));
...
await settle(200);
check('the bin deletes a piece', !(await kinds()).includes('Equation') && (await kinds()).length === 7);
...
check('the notes are all there', after.length === 7 && after.at(-2) === 'Text', after.join(' | '));
