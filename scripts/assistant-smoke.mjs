...
check('with no key it asks for one', await page.getByText('Connect an Anthropic account').isVisible());
check('and the compose box waits for it', await page.getByRole('textbox', { name: 'Ask Claude' }).isDisabled());
...
check('a saved key opens the compose box', await page.getByRole('textbox', { name: 'Ask Claude' }).isEnabled());
...
check('and puts the caret in the box', await page.evaluate(() => document.activeElement?.getAttribute('aria-label') === 'Ask Claude'));
...
await page.getByRole('textbox', { name: 'Ask Claude' }).fill('And the load-balancing loss?');
await page.keyboard.press('Enter');
...
await page.getByRole('textbox', { name: 'Ask Claude' }).focus();
